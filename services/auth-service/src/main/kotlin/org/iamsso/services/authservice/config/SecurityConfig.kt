...
class SecurityConfig(private val jwtKeyProvider: JwtKeyProvider) {
    ...
    fun oauthFilterChain(http: HttpSecurity): SecurityFilterChain =
        http
            .securityMatcher("/oauth2/**", "/.well-known/**", "/userinfo", "/actuator/**", "/login", "/mfa-challenge", "/mfa-challenge/**", "/forgot-password", "/reset-password", "/api/v1/sessions/logout", "/api/v1/sessions/logout-all")
            .csrf { it.disable() }
